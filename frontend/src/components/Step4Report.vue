            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content done, section may still be in progress) -->
...
const matchSectionContent = (text, headerPatterns) => {
  for (const headerPattern of headerPatterns) {
    const match = text.match(new RegExp(`###\\s*${headerPattern}[\\s\\S]*?\\n([\\s\\S]*?)(?=\\n###|$)`))
    if (match) return match[1]
  }
  return null
}

const parseNumberedList = (sectionText) => {
  if (!sectionText) return []
  return sectionText
    .split('\n')
    .filter((line) => line.match(/^\d+\./))
    .map((line) => {
      const quotedMatch = line.match(/^\d+\.\s*"?(.+?)"?\s*$/)
      return quotedMatch
        ? quotedMatch[1].replace(/^"|"$/g, '').trim()
        : line.replace(/^\d+\.\s*/, '').trim()
    })
    .filter(Boolean)
}

const parseRelationLines = (sectionText) => {
  if (!sectionText) return []
  return sectionText
    .split('\n')
    ...
    .map((line) => {
      ...
      return match
        ? { source: match[1].trim(), relation: match[2].trim(), target: match[3].trim() }
        : null
    })
    .filter(Boolean)
}
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // Special case when first question is prefixed with "1."
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          if (interview.redditAnswer && !isPlaceholder(interview.redditAnswer)) {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && !isPlaceholder(interview.twitterAnswer)) {
            interview.redditAnswer = interview.twitterAnswer
          }
        } else if (!twitterMatch && !redditMatch) {
          // No platform markers (legacy): treat whole block as answer
            ...
            (expandedHistorical.value ? props.result.historicalFacts : props.result.historicalFacts.slice(0, INITIAL_SHOW_COUNT)).map((fact, i) => 
              h('div', { class: 'fact-item historical', key: i }, [
                ...
                h('div', { class: 'fact-content' }, [
                  // Try to extract time range [time - time]
                  (() => {
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "Question X:" (Chinese format, new backend)
      // 2. "1. " or "\n1. " (numeric dot, legacy)
      let matches = []
      let match
      ...
      // Return whole block if no/single numbering found
      if (matches.length <= 1) {
    ...
    // Check dual-platform answers for a question (filter placeholders)
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Both platforms have real distinct answers
...
// Current active step (for header display)
...
const renderMarkdown = (content) => {
  ...
  // Strip leading ## heading; section title shown outside
  ...
  // Trim whitespace between list items
  html = html.replace(/<\/li>\s+<li/g, '</li><li')
  // Trim whitespace after list open tags
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // Trim whitespace before list close tags
  ...
  // Process line breaks - blank lines to paragraphs, single breaks to <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Trim <p><br> before block elements (extra blank lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Collapse consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Trim <br> before paragraph after block element
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix non-contiguous ordered list numbering when single-item <ol> blocks are separated
...
const getLogLevelClass = (log) => {
  ...
  // INFO uses default color, not marked success
  return ''
}
...
const extractFinalContent = (response) => {
  ...
  // Try to extract content inside <final_answer>
  ...
  // Try content after Final Answer: (multiple formats)

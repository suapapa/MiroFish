      ...
      >
        ...
        <div class="card-files-wrapper">
          ...
          <div class="files-list" v-if="project.files && project.files.length > 0">
            ...
            <!-- Show hint when more files exist -->
...
const containerStyle = computed(() => {
  ...
  // Height = rows * cardHeight + gaps + bottom padding
...
// Format history entry ID (simulation or project)
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update pending target state (always record latest intent)
        pendingState = shouldExpand
        
        // Clear prior debounce timer (new scroll intent wins)
        if (expandDebounceTimer) {
          clearTimeout(expandDebounceTimer)
          expandDebounceTimer = null
        }
        
        // If animating, record state and apply after animation
        if (isAnimating) return
        
        // Skip if target equals current state
        ...
        // Expand delay 50ms, collapse 200ms for stability
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check pending state still needed (may be superseded)
          ...
          // Unlock after animation; check pending state changes
          setTimeout(() => {
            isAnimating = false
            
            // After animation, check new pending state
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Short delay before apply to avoid rapid toggling
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // rootMargin shrinks bottom; need more scroll to expand
...
// Reload data when route returns to home
...
/* Square grid via CSS background pattern with fixed spacing */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
...
.cards-container {
  ...
  /* min-height computed in JS from card count */
}

    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Simulation rounds config - shown only after config is generated and rounds are computed -->
...
// Log dedup: track last emitted key info
...
// Compute auto-generated rounds from config (no hardcoded default)
...
// Auto-start or resume simulation preparation
...
const startPolling = () => {
  if (pollSSE) return
  pollSSE = createSSE(`/api/simulation/${props.simulationId}/prepare/stream`, {
    onUpdate: (data) => {
      void handlePrepareUpdate(data)
    },
    onComplete: (data) => {
      void handlePrepareUpdate(data)
    },
    onError: (err) => {
      console.warn('Prepare SSE stream error:', err)
    }
  })
}
...
const startProfilesPolling = () => {
  if (profilesSSE) return
  profilesSSE = createSSE(`/api/simulation/${props.simulationId}/profiles/stream?platform=reddit`, {
    onUpdate: (data) => {
      handleProfilesUpdate(data)
    },
    onComplete: (data) => {
      handleProfilesUpdate(data)
    },
    onError: (err) => {
      console.warn('Profiles SSE stream error:', err)
    }
  })
}
...
const handlePrepareUpdate = async (data) => {
...
const handleProfilesUpdate = (data) => {
  ...
  // Emit profile generation progress logs (only when count changes)
...
const startConfigPolling = () => {
  if (configSSE) return
  configSSE = createSSE(`/api/simulation/${props.simulationId}/config/stream`, {
    onUpdate: (data) => {
    ...
    onError: (err) => {
      console.warn('Config SSE stream error:', err)
    }
  })
}
...
const handleConfigUpdate = (data) => {
  // Emit config generation phase logs (avoid duplicates)
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        // Config not ready yet; start polling

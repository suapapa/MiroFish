...
const wasSimulating = ref(false) // Track whether was simulating
...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Show hint when transitioning from simulating to idle
...
const renderGraph = () => {
  ...
  // Process edges; count and index edges between node pairs
  const edgePairCount = {}
  ...
  tempEdges.forEach(e => {
    ...
    // Whether edge direction matches normalized order (source UUID < target UUID)
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    // Compute curvature: spread multiple edges, straight for single
    let curvature = 0
    if (totalCount > 1) {
      ...
      // Keeps edges in same reference frame to avoid overlap
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  // Simulation - adjust node spacing dynamically by edge count
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Adjust link distance by edge count between pair
    ...
    // Add center force to cluster isolated node groups
  ...
  const getLinkPath = (d) => {
    ...
    // Detect self-loop
    if (d.isSelfLoop) {
      // Self-loop: arc from node and back
      const loopRadius = 30
      // Start from node right, loop back
    ...
    // Control point from edge count and distance
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Perpendicular offset scaled by distance for visible curves
  ...
  // Curve midpoint for label placement
  ...
  // Link labels background (white for readability)
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Record position only; don't restart sim (click vs drag)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // Detect real drag start (movement over threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Restart sim only on first real drag
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Let sim settle only after real drag
        if (d._isDragging) {

...
TOOL_DESC_INSIGHT_FORGE = get_prompt("report_agent.tool_desc_insight_forge")

TOOL_DESC_PANORAMA_SEARCH = get_prompt("report_agent.tool_desc_panorama_search")

TOOL_DESC_QUICK_SEARCH = get_prompt("report_agent.tool_desc_quick_search")

TOOL_DESC_INTERVIEW_AGENTS = get_prompt("report_agent.tool_desc_interview_agents")

# ──Outline planning prompt──

PLAN_SYSTEM_PROMPT = get_prompt("report_agent.plan_system")

PLAN_USER_PROMPT_TEMPLATE = get_prompt("report_agent.plan_user")

# ──Chapter generation prompt──

SECTION_SYSTEM_PROMPT_TEMPLATE = get_prompt("report_agent.section_system")

SECTION_USER_PROMPT_TEMPLATE = get_prompt("report_agent.section_user")

# ── ReACT in-loop message template ──

REACT_OBSERVATION_TEMPLATE = get_prompt("report_agent.react_observation")

REACT_INSUFFICIENT_TOOLS_MSG = get_prompt("report_agent.react_insufficient_tools")

REACT_INSUFFICIENT_TOOLS_MSG_ALT = get_prompt("report_agent.react_insufficient_tools_alt")

REACT_TOOL_LIMIT_MSG = get_prompt("report_agent.react_tool_limit")

REACT_UNUSED_TOOLS_HINT = get_prompt("report_agent.react_unused_tools_hint")

REACT_FORCE_FINAL_MSG = get_prompt("report_agent.react_force_final")

# ── Chat prompt ──

CHAT_SYSTEM_PROMPT_TEMPLATE = get_prompt("report_agent.chat_system")

CHAT_OBSERVATION_SUFFIX = get_prompt("report_agent.chat_observation_suffix")
    ...
    def plan_outline(
        ...
        try:
            ...
            return ReportOutline(
                ...
                sections=[
                    ReportSection(title="Forecast Scenario and Key Findings"),
        ...
        for iteration in range(max_iterations):
            ...
            if has_tool_calls and has_final_answer:
                ...
                if conflict_retries <= 2:
                    ...
                    messages.append({
                        "role": "user",
                        "content": (
                            "[Format Error] Your reply included both a tool call and a Final Answer in the same response, which is not allowed.\n"
                            "Each reply may do only one of the following:\n"
                            "- Call one tool (output a single <tool_call> block, without Final Answer)\n"
            ...
            if has_final_answer:
                ...
                if tool_calls_count < min_tool_calls:
                    ...
                    unused_hint = f"(These tools have not been used yet and are recommended: {', '.join(unused_tools)})" if unused_tools else ""
                    messages.append({
                        "role": "user",

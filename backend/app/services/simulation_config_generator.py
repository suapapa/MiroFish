...
class SimulationConfigGenerator:
    ...
    def generate_config(
        ...
        logger.info(
            f"Simulation config generation completed: {len(params.agent_configs)} Agent configs"
        )
    ...
    def _assign_initial_post_agents(
        ...
        for post in event_config.initial_posts:
            ...
            if matched_agent_id is None:
                logger.warning(
                    f"No matching Agent found for type '{poster_type}', using the highest-influence Agent"
                )
                if agent_configs:
    ...
    def _generate_agent_configs_batch(
        ...
        try:
            ...
            logger.warning(
                f"Agent config batch LLM generation failed: {e}, falling back to rule-based generation"
            )
            llm_configs = {}

"""Graph pagination utilities (Graphiti/FalkorDB, self-hosted).

node/edge list APIs use UUID cursor pagination.
This module wraps auto-pagination (with per-page retry) and returns full lists transparently.
"""
...
_MAX_NODES = Config.GRAPH_MAX_NODES
_MAX_EDGES = Config.GRAPH_MAX_EDGES
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """Single-page fetch with exponential backoff retry. Retries only transient network/IO errors."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """Paginate graph nodes, returning at most max_items (default 2000). Each page request is retried."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Paginate graph edges, returning at most max_items (default 10000). Each page request is retried."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
